...
class AdaptiveHuffman:
    ...
    def decode(self):
        self.bit_seq = bitarray(endian=sys.byteorder)
        self.bit_seq.frombytes(self.byte_seq)
        self.current_bit_seq_idx = 0
        self.progressbar = ShadyBar('encoding', max=self.bit_seq.length(),
                                    suffix='%(percent).1f%% - %(elapsed_td)ss')
        ...
        # XXX: There might has a better way to replace this implement.
        remaining_bits_length = int(self.read_bit(3).to01(), 2)
        del self.bit_seq[-remaining_bits_length:]
        self.progressbar.next(remaining_bits_length)
        bit_seq_len = self.bit_seq.length()

        ret = []
        while self.current_bit_seq_idx < bit_seq_len:
            current_node = self.tree  # go to root
            while current_node.left or current_node.right:
                bit = self.read_bit(1)[0]
                current_node = current_node.right if bit else current_node.left
            if current_node.data == NYT:
                first_appearance = True
                # Convert fixed code into integer
                bits = self.read_bit(self.exp)
                dec = ord(bits.tobytes())
                if dec < self.rem:
                    bits.extend(self.read_bit(1))
                    dec = ord(bits.tobytes())
                else:
                    dec += self.rem
    ...
    def read_bit(self, n):
        """For decoder, get the first n bits in self.bit_seq and move
        self.current_bit_seq_idx forward for n.
        """
        self.progressbar.next(n)
        bits = self.bit_seq[self.current_bit_seq_idx:self.current_bit_seq_idx + n]
        self.current_bit_seq_idx += n
        return bits
